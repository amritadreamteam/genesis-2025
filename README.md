...
2. **Create directories named `task-##`** (where `##` corresponds to the task number). All work, files, screenshots, code, documents, and deliverables for each task **must be placed inside the correct corresponding directory**.
...
If you are using **macOS**, you don’t need to install Linux—simply set up [Homebrew](https://brew.sh/) as your package manager to continue with the tasks.
